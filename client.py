...
def main(
    ...
    # If no server arguments are provided, prefer the bundled config, otherwise enable auto-discovery
    if not (mcp_server or mcp_server_url or servers_json or auto_discovery):
        if os.path.exists(DEFAULT_SERVERS_FILE):
            servers_json = DEFAULT_SERVERS_FILE
        else:
            auto_discovery = True
...
async def async_main(mcp_server, mcp_server_url, servers_json, auto_discovery, model, host):
    ...
    default_servers_path = DEFAULT_SERVERS_FILE if os.path.exists(DEFAULT_SERVERS_FILE) else None
    ...
    if servers_json:
        ...
        # If neither is provided, prefer the bundled server list, then Claude's config
        if not mcp_server and not mcp_server_url:
            if default_servers_path:
                console.print(f"[cyan]Using bundled server configuration at {default_servers_path}[/cyan]")
                config_path = default_servers_path
            elif os.path.exists(DEFAULT_CLAUDE_CONFIG):
                console.print(f"[cyan]Auto-discovering servers from Claude's config at {DEFAULT_CLAUDE_CONFIG}[/cyan]")
                auto_discovery_final = True
            else:
